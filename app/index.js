...
const formatTaxTime = (ms) => {
  if (ms == null) return '72h after income starts';
...
function TaxStatusCard({ tax, title, incomeLabel, onPay, balance }) {
  ...
  return (
    <Card style={[styles.taxStatusCard, tax.suspended && styles.taxStatusCardOverdue]}>
      <View style={styles.taxStatusTop}>
        <IconBubble tone={tax.suspended ? 'red' : 'blue'} size={48}>%</IconBubble>
        <View style={styles.flex}>
          <Text style={styles.taxStatusTitle}>{title}</Text>
          <Text style={[styles.taxStatusSub, tax.suspended && styles.negative]}>
            {tax.suspended
              ? incomeLabel + ' suspended until this tax is paid.'
              : 'Next payment in ' + formatTaxTime(tax.timeRemainingMs)}
          </Text>
        </View>
        {tax.suspended && <View style={styles.taxHoldBadge}><Text style={styles.taxHoldBadgeText}>HOLD</Text></View>}
      </View>
      <View style={styles.taxStatusBottom}>
        <View>
          <Text style={styles.taxStatusLabel}>ACCRUED</Text>
          <Text style={styles.taxStatusAmount}>{compactMoney(tax.amount, 2)}</Text>
        </View>
        <View style={styles.alignRight}>
          <Text style={styles.taxStatusLabel}>RATE</Text>
          <Text style={styles.taxStatusRate}>{Math.round(tax.rate * 100)}%</Text>
        </View>
      </View>
      {due && (
        <Button
...
function Earnings({ game }) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      <Header title="Earnings" subtitle="Build your starting capital yourself." />
        ...
        <View style={styles.heroBottom}>
          <View style={styles.flex}>
            {game.hasPassiveSource ? (
              ...
              <>
                <Text style={styles.heroMiniLabel}>NEXT STEP</Text>
                <Text style={styles.heroMiniValue}>Build an income source</Text>
              </>
            )}
          </View>
...
function Work({ game }) {
  ...
  const remainingMs = Math.max(0, (game.jobReadyAt || 0) - now);
  const remainingSeconds = Math.ceil(remainingMs / 1000);

  return (
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      <Header title="Work" subtitle="Jobs are active income. No free money while you do nothing." />

      {game.selectedJob ? (
        <View style={styles.workHero}>
          <Text style={styles.workHeroKicker}>CURRENT JOB</Text>
          <Text style={styles.workHeroTitle}>{game.selectedJob.icon} {game.selectedJob.name}</Text>
          <Text style={styles.workHeroPay}>+{compactMoney(game.jobShiftPay, 2)}</Text>
          <Text style={styles.workHeroSub}>per completed shift</Text>
          <Pressable
            disabled={!game.jobReady}
            onPress={() => {
              tapHaptic();
              game.workJob();
              setNow(Date.now());
            ...
            style={({ pressed }) => [
              styles.workShiftButton,
              !game.jobReady && styles.workShiftButtonDisabled,
              pressed && game.jobReady && styles.pressed,
            ]}
          >
            <Text style={[styles.workShiftButtonText, !game.jobReady && styles.workShiftButtonTextDisabled]}>
              {game.jobReady ? 'Work shift' : 'Ready in ' + remainingSeconds + 's'}
            </Text>
          </Pressable>
        </View>
      ) : (
        <Card style={styles.workEmptyCard}>
          <IconBubble tone="gold" size={58}>▣</IconBubble>
          <Text style={styles.workEmptyTitle}>Choose your first job.</Text>
          <Text style={styles.workEmptyCopy}>Jobs only pay when you work a shift. Passive income starts later from businesses, property and dividend stocks.</Text>
        </Card>
      )}

      <SectionTitle title="Jobs" subtitle="Higher-value careers unlock as your net worth grows." />
      {game.jobs.map((job) => {
        const active = game.activeJobId === job.id;
        const unlocked = game.netWorth >= job.unlockNetWorth;
        const shiftPay = job.incomePerSec * 15;
        return (
          <Card key={job.id} style={active && styles.activeCard}>
            <View style={styles.row}>
              <IconBubble tone={active ? 'mint' : unlocked ? 'blue' : 'gold'}>{job.icon}</IconBubble>
              <View style={styles.flex}>
                <Text style={styles.cardTitle}>{job.name}</Text>
                <Text style={styles.cardSub}>
                  {unlocked ? compactMoney(shiftPay, 2) + ' per shift' : 'Unlock at ' + compactMoney(job.unlockNetWorth, 1) + ' net worth'}
                </Text>
              </View>
              <Button
                small
                secondary={!active}
                label={active ? 'Selected' : unlocked ? 'Choose' : 'Locked'}
      ...
      <Card style={styles.workTipCard}>
        <Text style={styles.workTipTitle}>How income works</Text>
        <Text style={styles.workTipText}>Tap earnings and job shifts are active income. Businesses, rent and stock dividends become passive only after you buy them.</Text>
      </Card>
    </ScrollView>
  );
}
        ...
        <Card key={property.id}>
          <View style={styles.row}>
            ...
            <View style={styles.alignRight}>
              ...
              <Text style={game.taxes.realEstate.suspended ? styles.negative : styles.positive}>
                {game.taxes.realEstate.suspended ? 'SUSPENDED' : '+' + compactMoney(property.rentPerSec * property.count, 2) + '/s'}
              </Text>
            </View>
          </View>
        </Card>
    ...
    <>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.businessSummary}>
          <Text style={styles.heroLabel}>{game.taxes.business.suspended ? 'Income suspended' : 'Total income per second'}</Text>
          <Text style={[styles.businessIncome, game.taxes.business.suspended && styles.negative]}>
            {game.taxes.business.suspended ? compactMoney(0, 2) : '+' + compactMoney(game.businessIncomePerSec, 2)}
          </Text>
        ...
        {game.businesses.map((business) => {
          ...
          return (
            <Card key={business.id}>
              ...
              <Text style={[styles.businessCardIncome, game.taxes.business.suspended && styles.negative]}>
                {game.taxes.business.suspended ? 'SUSPENDED' : compactMoney(business.incomePerSec, 2)}
                {!game.taxes.business.suspended && <Text style={styles.businessCardSuffix}> per sec</Text>}
              </Text>
            </Card>
          );
        })}
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.modalPage}>
          ...
          <View style={styles.companyHero}>
            ...
            <Text style={[styles.companyHeroIncome, game.taxes.business.suspended && styles.negative]}>
              {game.taxes.business.suspended ? 'INCOME SUSPENDED' : '+' + compactMoney(business.incomePerSec, 2) + '/s'}
            </Text>
            <Text style={styles.companyHeroLabel}>{game.taxes.business.suspended ? 'Pay company tax to resume operations' : 'current company profit'}</Text>
          </View>
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
      ...
      <Card style={styles.statsCard}>
        ...
        <ProfileStat label="Passive income" value={game.hasPassiveSource ? compactMoney(game.passivePerSec, 2) + '/s' : 'Not built yet'} last />
      </Card>
...
const styles = StyleSheet.create({
  ...
  taxStatusCard: { backgroundColor: C.blueSoft, borderWidth: 1, borderColor: '#D9E6FA' },
  taxStatusCardOverdue: { backgroundColor: C.redSoft, borderColor: '#F4C5C5' },
  taxStatusTop: { flexDirection: 'row', alignItems: 'center', gap: 12 },
  taxStatusTitle: { color: C.text, fontSize: 17, fontWeight: '850' },
  taxStatusSub: { color: C.muted, fontSize: 13, lineHeight: 18, marginTop: 3 },
  taxStatusBottom: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 15, marginBottom: 14, paddingTop: 13, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#CAD7E8' },
  taxStatusLabel: { color: C.muted, fontSize: 9, fontWeight: '850', letterSpacing: 0.8 },
  taxStatusAmount: { color: C.text, fontSize: 22, fontWeight: '900', marginTop: 4 },
  taxStatusRate: { color: C.text, fontSize: 17, fontWeight: '900', marginTop: 4 },
  taxHoldBadge: { borderRadius: 10, backgroundColor: C.red, paddingHorizontal: 8, paddingVertical: 5 },
  taxHoldBadgeText: { color: C.white, fontSize: 9, fontWeight: '900', letterSpacing: 0.7 },

  workHero: { minHeight: 270, borderRadius: 30, backgroundColor: '#172F3B', padding: 24, marginBottom: 16, justifyContent: 'center' },
  workHeroKicker: { color: '#88A9B8', fontSize: 10, fontWeight: '900', letterSpacing: 1.2 },
  workHeroTitle: { color: C.white, fontSize: 26, fontWeight: '900', marginTop: 8 },
  workHeroPay: { color: C.white, fontSize: 38, fontWeight: '900', marginTop: 24, letterSpacing: -1 },
  workHeroSub: { color: '#9BB7C3', fontSize: 13, marginTop: 2, marginBottom: 18 },
  workShiftButton: { minHeight: 52, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: C.mint },
  workShiftButtonDisabled: { backgroundColor: 'rgba(255,255,255,0.10)' },
  workShiftButtonText: { color: C.white, fontSize: 16, fontWeight: '900' },
  workShiftButtonTextDisabled: { color: '#8DA4AE' },
  workEmptyCard: { alignItems: 'center', paddingVertical: 30 },
  workEmptyTitle: { color: C.text, fontSize: 22, fontWeight: '900', marginTop: 14 },
  workEmptyCopy: { color: C.muted, fontSize: 14, lineHeight: 20, textAlign: 'center', marginTop: 7, maxWidth: 310 },
  workTipCard: { backgroundColor: C.mintSoft, marginTop: 18 },
  workTipTitle: { color: C.text, fontSize: 16, fontWeight: '850' },
  workTipText: { color: C.muted, fontSize: 13, lineHeight: 19, marginTop: 5 },
  ...
  navIconWrap: { width: 34, height: 31, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
  navIconWrapActive: { backgroundColor: C.mintSoft },
  navIcon: { fontSize: 21, color: C.faint, fontWeight: '700' },
  navIconActive: { color: C.mint },
  navLabel: { fontSize: 10, color: C.faint, marginTop: 4, fontWeight: '650' },
